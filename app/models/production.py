...
class BOMItem(Base):
    ...
    work_center_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("work_centers.id", ondelete="RESTRICT"), index=True
    )
...
class ProductionOrder(Base, TimestampMixin):
    ...
    bom_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("boms.id", ondelete="RESTRICT"), nullable=False, index=True
    )
    so_item_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("sales_order_items.id", ondelete="SET NULL"), index=True
    )
    work_center_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("work_centers.id", ondelete="SET NULL"), index=True
    )
